	...
	$.fn.timepick = function() {
		...
		if (this.hasClass('num'))
		{
			...
			if (s > 0)
			{
				...
				v = 'value="'+d.getFullYear()+'-'+(d.getMonth()+1)+'-'+d.getDate()+' '+d.getHours()+':'+d.getMinutes()+':'+d.getSeconds()+'"';
			}
			var fake = $('<input type="text" '+v+' />');
			var that = this;
			that.css({'position':'absolute','left':'-5000px','width':'10px'}).after(fake);
			/**/
			fake.datetimepicker(
			{
				dateFormat:'yy-mm-dd',
...
// datetimepicker: Trent Richardson [http://trentrichardson.com]
	...
	*/
	var Timepicker = function() {
		...
		this.regional[''] = { // Default regional settings
			currentText: '<?php echo L('Now')?>',
			closeText: '<?php echo L('close')?>',
			amNames: ['AM', 'A'],
			pmNames: ['PM', 'P'],
			timeFormat: 'HH:mm:ss',
			timeSuffix: '',
			timeOnlyTitle: '<?php echo L('choose_Time')?>',
			timeText: '<?php echo L('Time')?>',
			hourText: '<?php echo L('Hour')?>',
			minuteText: '<?php echo L('Minute')?>',
			secondText: '<?php echo L('Second')?>',
			millisecText: '<?php echo L('Millisecond')?>',
			timezoneText: '<?php echo L('Time_Zone')?>',
			isRTL: false
		};
